...
@registry.ROI_RELATION_PREDICTOR.register("UP-Net")
class UnionPrototypeEmbeddingNetwork(nn.Module):
    def __init__(self, config, in_channels):
        super(UnionPrototypeEmbeddingNetwork, self).__init__()
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        fusion_so = []
        entity_sub, entity_obj = self.W_sub(self.obj_embed.weight), self.W_obj(self.obj_embed.weight)
        for pair_idx, sub_rep, obj_rep, entity_pred in zip(rel_pair_idxs, sub_reps, obj_reps, entity_preds):
            s_embed = entity_sub[entity_pred[pair_idx[:, 0]]]  #  Ws x ts
            o_embed = entity_obj[entity_pred[pair_idx[:, 1]]]  #  Wo x to
        ...
        gate_sem_union = torch.sigmoid(self.gate_union(cat((fusion_so, sem_pred), dim=-1)))  # gp
        rel_rep = fusion_so + sem_pred * gate_sem_union  #  F(s,o) - gp · h(xu)   i.e., r = F(s,o) - up

        gate_sem_pred = torch.sigmoid(self.gate_pred(cat((rel_rep, sem_pred), dim=-1)))  # gp
        rel_rep = rel_rep - sem_pred * gate_sem_pred  #  F(s,o) - gp · h(xu)   i.e., r = F(s,o) - up
        ...
        if self.training:
            def triplet_loss(anchor, positive, negative, margin=1.0):
                loss = nn.TripletMarginLoss(margin=margin, p=2)(anchor, positive, negative)
                return loss.mean()
            ...
            new_rel_pair_idxs = [entity_pred[pair_idx] for pair_idx, entity_pred in zip(rel_pair_idxs, entity_preds)]
            rel_pairs, rel_labels = torch.cat(new_rel_pair_idxs), torch.cat(rel_labels)

            unique_labels = torch.unique(rel_labels)
            unique_labels = unique_labels[unique_labels != 0]
            for label in unique_labels:
                same_rel_pairs = rel_pairs[rel_labels == label]
                                        
                # Sub-Obj (Rel) triplet pairs
                anchor_rel_reps = rel_rep[rel_labels == label]
                for rel_pair, anchor_rel_rep in zip(same_rel_pairs, anchor_rel_reps):
                    same_pair_mask = (rel_pairs[:, 0] == rel_pair[0]) & (rel_pairs[:, 1] == rel_pair[1])

                    positive_mask = (same_pair_mask) & (rel_labels == label)
                    negative_mask = (same_pair_mask) & (rel_labels != label)

                    if positive_mask.sum() > 0 and negative_mask.sum() > 0:
                        positive_rel_samples = rel_rep[positive_mask]
                        negative_rel_samples = rel_rep[negative_mask]
                        triplet_rel_pairs.append((anchor_rel_rep, positive_rel_samples.mean(dim=0), negative_rel_samples.mean(dim=0)))
                
            rel_triplet_losses = []
            for anchor, positive, negative in triplet_rel_pairs:
                rel_triplet_losses.append(triplet_loss(anchor, positive, negative, margin))

            rel_triplet_loss = torch.stack(rel_triplet_losses).mean() if rel_triplet_losses else torch.tensor(0.0)
            add_losses.update({"triplet_loss": rel_triplet_loss})
            # ### end
